 ...
 * @author Aaron Faanes
 * @param <C>
 *            The type of the children this view will create
 * @param <E>
 *            The type that the listModel contains
 */
public abstract class Repeater<C extends Component, E> {
	...
	private final ListDataListener listener = new ListDataListener() {
		...
		public void intervalAdded(final ListDataEvent e) {
			for (int index = e.getIndex0(); index <= e.getIndex1(); index++) {
				final E childValue = Repeater.this.getElementAt(index);
				final C childComponent = Repeater.this.createComponent(childValue);
		...
		public void intervalRemoved(final ListDataEvent e) {
			for (int index = e.getIndex1(); index >= e.getIndex0(); index--) {
				final C childComponent = Repeater.this.getComponentAt(index);
	...
	/**
	 * Creates a Repeater that will populate the given parent with data from the
	 * given ListModel.
	 * <p>
	 * Notice that all children in the parent will be removed.
	 * 
	 * @param parent
	 *            The parent container to populate
	 * @param model
	 *            The model that will provide the data to populate the parent
	 */
	public Repeater(final Container parent, final ListModel model) {
	...
	public final void refresh() {
		...
		for (int i = 0; i < this.listModel.getSize(); i++) {
			final C component = this.createComponent(this.getElementAt(i));
	 ...
	 * @param index
	 *            The index where the component should be added
	 * @param childValue
	 *            The data that the component represents
	 */
