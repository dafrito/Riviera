 ...
 * is "ready" to use. Concrete subclasses must override the {@code startup}
 * method.
 * <p>
 ...
 * calling {@code ApplicationContext} {@link #exit}.
 * <p>
 ...
 * {@code resources/MyApplication.properties}). This ResourceBundle contains
...
public abstract class Application extends AbstractBean {
     ...
     * calls the new {@code Application's} {@code startup} method. The {@code
     * launch} method is typically called from the Application's {@code main}:
     * 
     * <pre>
     * public static void main(String[] args) {
     *     Application.launch(MyApplication.class, args);
     ...
     *            the {@code Application} class to launch
     * @param args
     *            {@code main} method arguments
     * @see #shutdown
        ...
        final Runnable doCreateAndShowGUI = new Runnable() {
            public void run() {
                try {
                    ...
                    final String msg = String.format("Application %s failed to launch", applicationClass);
                    Application.logger.log(Level.SEVERE, msg, e);
    ...
    static <T extends Application> T create(final Class<T> applicationClass) throws Exception {
        ...
        if (!Beans.isDesignTime()) {
             ...
             */
            final String key = "Application.lookAndFeel";
            final String lnfResource = appResourceMap.getString(key);
            final String lnf = (lnfResource == null) ? "system" : lnfResource.toLowerCase();
            try {
                if (lnf.equalsIgnoreCase("system")) {
                    ...
                    try {
                        // Try using the value directly as a class-name
                        UIManager.setLookAndFeel(lnf);
                    } catch (final Exception e) {
                        // Try searching for look and feels that match the
                        // specified class name
                        for (final LookAndFeelInfo info : UIManager.getInstalledLookAndFeels()) {
                            if (info.getClassName().toLowerCase().contains(lnf)) {
     ...
     * <p>
     * This method is called by the static {@code launch} method, before {@code
     * startup} is called. Subclasses that want to do any initialization work
     * before {@code startup} must override it. The {@code initialize} method
     * runs on the event dispatching thread.
     * <p>
     * By default initialize() does nothing.
     * 
     * @param args
     *            the main method's arguments.
     * @see #launch
     * @see #startup
     * @see #shutdown
     */
    ...
    private static class NotifyingEvent extends PaintEvent implements ActiveEvent {
         ...
         * 
         */
        private static final long serialVersionUID = 791064584368983950L;
        private boolean dispatched = false;
        private boolean qEmpty = false;

        NotifyingEvent(final Component c) {
     ...
     * triggered by non-GUI code, and {@code canExit}, {@code willExit} methods
     * must guard against the possibility that the {@code eventObject}
     * argument's {@code source} is not a {@code Component}.
     * 
     * @param event
     *            the EventObject that triggered this call or null
     * @see #addExitListener
    ...
    public void exit(final EventObject event) {
        for (final ExitListener listener : this.exitListeners) {
        ...
        try {
            for (final ExitListener listener : this.exitListeners) {
                try {
                    ...
                    Application.logger.log(Level.WARNING, "ExitListener.willExit() failed", e);
                }
            }
            this.shutdown();
        } catch (final Exception e) {
            Application.logger.log(Level.WARNING, "unexpected error in Application.shutdown()", e);
        } finally {
            this.end();
        }
    }
    ...
    /**
     * Called by {@link #exit exit} to terminate the application. Calls {@code
    ...
    /**
     * Give the Application a chance to veto an attempt to exit/quit. An {@code
     * ExitListener's} {@code canExit} method should return false if there are
     * pending decisions that the user must make before the app exits. A typical
     ...
     * 
     * @param listener
     *            the {@code ExitListener}
     * @see #removeExitListener
     * @see #getExitListeners
     */
    public void addExitListener(final ExitListener listener) {
     ...
     * @see #getExitListeners
     */
    public void removeExitListener(final ExitListener listener) {
    ...
    public ExitListener[] getExitListeners() {
        final int size = this.exitListeners.size();
     ...
     * it would be by the {@link #launch launch} method. However it's {@code
     * initialize} and {@code startup} methods are not run.
     * 
     * @param applicationClass
     ...
     */
    public static synchronized <T extends Application> T getInstance(final Class<T> applicationClass) {
     ...
     * <i>placeholder</i> Application object provides access to an {@code
    ...
    private static class NoApplication extends Application {
        protected NoApplication() {
            final ApplicationContext ctx = this.getContext();
