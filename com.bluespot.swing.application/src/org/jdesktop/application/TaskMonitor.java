...
public class TaskMonitor extends AbstractBean {
     ...
     */
    public TaskMonitor(final ApplicationContext context) {
        ...
        for (final TaskService taskService : context.getTaskServices()) {
    ...
    public void setForegroundTask(final Task foregroundTask) {
        ...
        final Task newTask = this.foregroundTask;
        if (newTask != null) {
            newTask.addPropertyChangeListener(this.taskPCL);
        }
        this.firePropertyChange("foregroundTask", oldTask, newTask);
    }
    ...
    public void setAutoUpdateForegroundTask(final boolean autoUpdateForegroundTask) {
        final boolean oldValue = this.autoUpdateForegroundTask;
     ...
     * DONE.
     */
    private void updateTasks(final List<Task> oldTasks, final List<Task> newTasks) {
        boolean tasksChanged = false; // has the "tasks" property changed?
        final List<Task> oldTaskQueue = this.copyTaskQueue();
        // Remove each oldTask that's not in the newTasks list from taskQueue
        for (final Task oldTask : oldTasks) {
            if (!(newTasks.contains(oldTask))) {
                if (this.taskQueue.remove(oldTask)) {
                    tasksChanged = true;
                }
            }
        }
        ...
        final Iterator<Task> tasks = this.taskQueue.iterator();
        while (tasks.hasNext()) {
        ...
        if (tasksChanged) {
            final List<Task> newTaskQueue = this.copyTaskQueue();
    ...
    private class ApplicationPCL implements PropertyChangeListener {
        public void propertyChange(final PropertyChangeEvent e) {
            ...
            if ("taskServices".equals(propertyName)) {
                final List<TaskService> oldList = (List<TaskService>) e.getOldValue();
                final List<TaskService> newList = (List<TaskService>) e.getNewValue();
                for (final TaskService oldTaskService : oldList) {
                    oldTaskService.removePropertyChangeListener(TaskMonitor.this.taskServicePCL);
                }
                for (final TaskService newTaskService : newList) {
    ...
    private class TaskServicePCL implements PropertyChangeListener {
        public void propertyChange(final PropertyChangeEvent e) {
            ...
            if ("tasks".equals(propertyName)) {
                final List<Task> oldList = (List<Task>) e.getOldValue();
                final List<Task> newList = (List<Task>) e.getNewValue();
    ...
    private class TaskPCL implements PropertyChangeListener {
        private void fireStateChange(final Task task, final String propertyName) {
        ...
        public void propertyChange(final PropertyChangeEvent e) {
            ...
            if ((task != null) && (task == TaskMonitor.this.getForegroundTask())) {
                ...
                if ("state".equals(propertyName)) {
                    final StateValue newState = (StateValue) (e.getNewValue());
                    switch (newState) {
                    case PENDING:
