 ...
 * dividerLocation} for {@code JSliderPanes} and the {@code selectedIndex} for
 * {@code JTabbedPanes} are defined by default. The {@code ApplicationContext}
 * {@link ApplicationContext#getSessionStorage getSesssionStorage} method
 ...
 * </pre>
 * 
 * In this example, the bounds of {@code mainFrame} as well the session state
 * for any of its {@code JSliderPane} or {@code JTabbedPane} will be saved when
 * the application shuts down, and restored when the applications starts up
...
public class SessionStorage {
    ...
    /**
     * Constructs a SessionStorage object. The following {@link Property
     * Property} objects are registered by default:
     * <p>
    ...
    private void checkSaveRestoreArgs(final Component root, final String fileName) {
        if (root == null) {
     ...
     */
    private String getComponentName(final Component c) {
        return c.getName();
    }
     ...
     * 
     * JFrame myFrame = new JFrame(); JPanel p = new JPanel() {}; // anonymous
     * JPanel subclass JButton myButton = new JButton();
    ...
    private String getComponentPathname(Component c) {
        ...
        while ((c.getParent() != null) && !(c instanceof Window) && !(c instanceof Applet)) {
            c = c.getParent();
            name = this.getComponentName(c);
            if (name == null) {
                final int n = c.getParent().getComponentZOrder(c);
                if (n >= 0) {
                    ...
                    SessionStorage.logger.warning("Couldn't compute pathname for " + c);
                    return null;
                }
            }
     ...
     */
    private void saveTree(final List<Component> roots, final Map<String, Object> stateMap) {
        final List<Component> allChildren = new ArrayList<Component>();
        for (final Component root : roots) {
            if (root != null) {
                final Property p = this.getProperty(root);
                if (p != null) {
                    final String pathname = this.getComponentPathname(root);
                    if (pathname != null) {
                        final Object state = p.getSessionState(root);
                        if (state != null) {
     ...
     * component is visited in breadth-first order: if a {@code Property}
     ...
     * 
     * @param root
     *            the root of the Component hierarchy to be saved.
     * @param fileName
     *            the {@code LocalStorage} filename.
     * @see #restore
     * @see ApplicationContext#getLocalStorage
     * @see LocalStorage#save
     * @see #getProperty(Component)
     */
    public void save(final Component root, final String fileName) throws IOException {
        ...
        final LocalStorage lst = this.getContext().getLocalStorage();
     ...
     */
    private void restoreTree(final List<Component> roots, final Map<String, Object> stateMap) {
        final List<Component> allChildren = new ArrayList<Component>();
        for (final Component root : roots) {
            if (root != null) {
                final Property p = this.getProperty(root);
                if (p != null) {
                    final String pathname = this.getComponentPathname(root);
                    if (pathname != null) {
                        final Object state = stateMap.get(pathname);
                        if (state != null) {
                            p.setSessionState(root, state);
                        } else {
                            SessionStorage.logger.warning("No saved state for " + root);
                        }
                    }
                }
            }
     ...
     * state loaded from a file using {@link LocalStorage#save
     * LocalStorage.load(fileName)}. Each component is visited in breadth-first
     * order: if a {@link #getProperty(Component) Property} exists for that
     * component, and the component has a {@link java.awt.Component#getName
     * name}, then its state is {@link Property#setSessionState restored}.
     * 
     * @param root
     *            the root of the Component hierarchy to be restored.
     * @param fileName
     *            the {@code LocalStorage} filename.
     * @see #save
     * @see ApplicationContext#getLocalStorage
     ...
     * @see #getProperty(Component)
     */
    public void restore(final Component root, final String fileName) throws IOException {
        this.checkSaveRestoreArgs(root, fileName);
        final LocalStorage lst = this.getContext().getLocalStorage();
        final Map<String, Object> stateMap = (Map<String, Object>) (lst.load(fileName));
        if (stateMap != null) {
    ...
    public interface Property {
         ...
         *            the Component.
         * @param state
         *            the value of the {@code sessionState} property.
         * @see #getSessionState
         */
    ...
    public static class WindowState {
        ...
        public void setGraphicsConfigurationBounds(final Rectangle gcBounds) {
     ...
     * {@code Frame} class, as well as their Swing counterparts: {@code JWindow}, {@code JDialog}, and {@code JFrame}.
     * 
     * @see #save
     * @see #restore
     * @see WindowState
    ...
    public static class WindowProperty implements Property {
         ...
         * 
         * @param c
         *            the {@code Window} whose bounds will be stored in a
         *            {@code WindowState} object.
         * @return the {@code WindowState} object
         * @see #setSessionState
         * @see WindowState
         */
        public Object getSessionState(final Component c) {
            ...
            final Rectangle gcBounds = (gc == null) ? null : gc.getBounds();
            ...
            if ((c instanceof JFrame) && (0 != (frameState & Frame.MAXIMIZED_BOTH))) {
                ...
                final Object r = ((JFrame) c).getRootPane().getClientProperty(clientPropertyKey);
         ...
         * {@code GraphicsConfiguration}) haven't changed, the number of screens
         * hasn't changed, and the {@link Window#isLocationByPlatform
         ...
         * matches the {@link WindowState#getGraphicsConfigurationBounds
         ...
         * 
         * @param c
         *            the Window whose state is to be restored
         * @param state
         *            the {@code WindowState} to be restored
         * @see #getSessionState
         * @see WindowState
         */
        public void setSessionState(final Component c, final Object state) {
            this.checkComponent(c);
            ...
            if (!w.isLocationByPlatform() && (state != null)) {
                final WindowState windowState = (WindowState) state;
                final Rectangle gcBounds0 = windowState.getGraphicsConfigurationBounds();
                final int sc0 = windowState.getScreenCount();
                final GraphicsConfiguration gc = c.getGraphicsConfiguration();
                final Rectangle gcBounds1 = (gc == null) ? null : gc.getBounds();
                final int sc1 = this.getScreenCount();
    ...
    /**
     * This Java Bean record the {@code selectedIndex} and {@code tabCount}
    ...
    public static class TabbedPaneProperty implements Property {
         ...
         * 
         * @param c
         *            the {@code JTabbedPane} whose selectedIndex will recoreded
        ...
        public Object getSessionState(final Component c) {
            this.checkComponent(c);
            final JTabbedPane p = (JTabbedPane) c;
         ...
         * 
         * @param c
         *            the JTabbedPane whose state is to be restored
         * @param state
         *            the {@code TabbedPaneState} to be restored
         * @see #getSessionState
         * @see TabbedPaneState
         */
        public void setSessionState(final Component c, final Object state) {
            ...
            final JTabbedPane p = (JTabbedPane) c;
            final TabbedPaneState tps = (TabbedPaneState) state;
     ...
     * orientation} properties of a {@code JSplitPane}. A {@code SplitPaneState}
     * object created by {@link SplitPaneProperty#getSessionState} and used to
     * restore the selected tab by {@link SplitPaneProperty#setSessionState}.
     * 
     ...
     * JSplitPane. One can override the default with the {@link #putProperty
     * putProperty} method.
     * 
     * @see SplitPaneState
     * @see #save
    ...
    public static class SplitPaneProperty implements Property {
         ...
         * 
         * @param c
         *            the {@code JSplitPane} whose dividerLocation will
         ...
         * 
         * @param c
         *            the JSplitPane whose state is to be restored
         * @param state
         *            the {@code SplitPaneState} to be restored
         * @see #getSessionState
         * @see SplitPaneState
         */
        public void setSessionState(final Component c, final Object state) {
    ...
    public static class TableState {
        ...
        private int[] copyColumnWidths(final int[] columnWidths) {
            if (columnWidths == null) {
                throw new IllegalArgumentException("invalid columnWidths");
            }
            final int[] copy = new int[columnWidths.length];
        ...
        public TableState(final int[] columnWidths) {
     ...
     * <p>
     * {@code TableProperty} is registered for {@code JTable.class} by default,
    ...
    public static class TableProperty implements Property {
         ...
         * 
         * @param c
         *            the {@code JTable} whose columnWidths will be saved in a
        ...
        public Object getSessionState(final Component c) {
            ...
            final int[] columnWidths = new int[table.getColumnCount()];
         ...
         * 
         * @param c
         *            the JTable whose column widths are to be restored
         * @param state
         *            the {@code TableState} to be restored
         * @see #getSessionState
         * @see TableState
         */
        public void setSessionState(final Component c, final Object state) {
            ...
            final JTable table = (JTable) c;
            final int[] columnWidths = ((TableState) state).getColumnWidths();
    ...
    private void checkClassArg(final Class cls) {
        if (cls == null) {
     ...
     * been registered, return null. To lookup the session state {@code
     * Property} for a {@code Component} use {@link #getProperty(Component)}.
     * <p>
     * Throws an {@code IllegalArgumentException} if {@code cls} is null.
     * 
     * @param cls
     *            the class to which the returned {@code Property} applies
    ...
    public Property getProperty(Class cls) {
        ...
        while (cls != null) {
            final Property p = this.propertyMap.get(cls);
            if (p != null) {
                return p;
            }
     ...
     * 
     * @param cls
     *            the class to which {@code property} applies.
     * @param property
     *            the {@code Property} object to register or null.
     * @see #getProperty(Component)
     * @see #getProperty(Class)
     * @see #save
     * @see #restore
     */
    public void putProperty(final Class cls, final Property property) {
        this.checkClassArg(cls);
     ...
     * </pre>
     * 
     * One can also create components that implement the {@code
     * SessionState.Property} interface directly.
     * 
     * @return if {@code Component c} implements {@code Session.Property}, then
     *         {@code c}, if {@code c} is a {@code JComponent} with a {@code
     *         Property} valued {@link javax.swing.JComponent#getClientProperty
     *         client property} under (client property key) {@code
     *         SessionState.Property.class}, then return that, otherwise return
     *         the value of {@code getProperty(c.getClass())}.
     *         <p>
     *         Throws an {@code IllegalArgumentException} if {@code Component c}
     *         is null.
     * 
     ...
     * @see #restore
     */
    public final Property getProperty(final Component c) {
        if (c == null) {
            throw new IllegalArgumentException("null component");
        }
        if (c instanceof Property) {
            ...
            if (c instanceof JComponent) {
                final Object v = ((JComponent) c).getClientProperty(Property.class);
