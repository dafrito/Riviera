...
public class LoggingPerspective extends AbstractPerspective {
	...
	public abstract static class LoggingPerspectiveAction extends PerspectiveAction {
		...
		public LoggingPerspectiveAction(final String name) {
			super(name);
		}
	...
	public static class UnwatchPackageAction extends LoggingPerspectiveAction {
		...
		public void actionPerformed(final ActionEvent e) {
			try {
				...
				for (final Logger logger : this.getPerspective().getLoggers()) {
				...
				this.getPerspective()
						.removeLogger(
								Dialogs.getSelection("Choose the package that you want to unwatch:", packages,
										"Unwatch Logger"));
			} catch (final CancelledException e1) {
				return;
			}
		}
	...
	public LoggingPerspective() {
		...
		new TabView<JComponent, Logger>(this.logPanel, this.loggers) {
			...
			@Override
			public JComponent createComponent(final Logger logger) {
				try {
					...
					logTree.addTreeSelectionListener(new TreeSelectionListener() {
						...
						public void valueChanged(final TreeSelectionEvent e) {
							final Tree<?> node = (Tree<?>) logTree.getLastSelectedPathComponent();
	...
	public LoggingPerspective(final String... logs) {
		this();
	...
	public void addLogger(final Logger logger, final Level level) {
	...
	public void addLogger(final String loggerName, final Level level) {
