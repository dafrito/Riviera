 ...
 * <p>
 * This allows insertion at an arbitrary node. You may enter and leave nodes to navigate
 * the tree. The walker has no concept of the tree as a whole, so you can spin off walkers
 * at any point.
 * <p>
 ...
 * <p>
 * I won't do it yet since I'm not sure how to do it without bloating the interface.
 * 
 * @author Aaron Faanes
 *
 * @param <T> The type of the tree we're walking
 */
public class TreeWalker<T> {
    
    private Tree<T> currentNode;
    
    public TreeWalker(Tree<T> node) {
    ...
    public Tree<T> appendAndEnter(T value) {
        return this.enter(this.append(value));
    }

    public Tree<T> appendAndLeave(T value) {
        this.append(value);
        return this.leave();
    }
    ...
    public Tree<T> enter(Tree<T> node) {
