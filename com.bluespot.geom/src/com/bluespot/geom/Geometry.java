...
public final class Geometry {
     ...
     * 
     * @param dimension the {@code Dimension} to trim. It will be modified by
     *        this operation
     * @see Geometry#growToSquare(java.awt.Dimension)
     */
    public static void trimToSquare(Dimension dimension) {
        int smallestAxis = Math.abs(Geometry.minAxis(dimension));
        // We use signum to preserve the sign of the original dimension.
        int width = smallestAxis * (int)Math.signum(dimension.width);
        int height = smallestAxis * (int)Math.signum(dimension.height);
     ...
     * 
     * @param dimension the {@code Dimension} to use in this operation
     * @return the smaller axis's value
     */
    public static int minAxis(Dimension dimension) {
     ...
     */
    public static int maxAxis(Dimension dimension) {
        return Math.abs(dimension.width) > Math.abs(dimension.height) ? dimension.width : dimension.height;
    }
     ...
     * 
     * @param rectangle the {@code Rectangle} to trim. It will be modified by
     *        this operation
     * @see Geometry#trimToSquare(Dimension)
     * @see Geometry#alignCenter(Rectangle, Point)
     */
    public static void trimToSquare(Rectangle rectangle) {
     ...
     * "flip" due to opposing signs changing the resulting size.
     * 
     * @param dimension The {@code Dimension} to grow. It will be modified by
     *        this operation
     * @see Geometry#trimToSquare(java.awt.Dimension)
     */
    public static void growToSquare(Dimension dimension) {
        int largestAxis = Math.abs(Geometry.maxAxis(dimension));
        // We use signum to preserve the sign of the original dimension.
        int width = largestAxis * (int)Math.signum(dimension.width);
        int height = largestAxis * (int)Math.signum(dimension.height);
        dimension.setSize(width, height);
    }
     ...
     * 
     * @param rectangle the {@code Rectangle} to grow. It will be modified by
     *        this operation
     * @see Geometry#growToSquare(Dimension)
     */
    public static void growToSquare(Rectangle rectangle) {
        Dimension dimension = rectangle.getSize();
     ...
     * </ul>
     * 
     * @param dimension the {@code Dimension} to scale. It will be modified by
     *        this operation.
     * @param multiplier the scale
     * @see Geometry#scale(Dimension, double, double)
     */
    public static void scale(Dimension dimension, double multiplier) {
     ...
     * </ul>
     * 
     * @param dimension the {@code Dimension} that will be modified by this
     *        operation
     * @param widthMultiplier The width's multiplier
     * @param heightMultiplier The height's multiplier
     */
    public static void scale(Dimension dimension, double widthMultiplier, double heightMultiplier) {
     ...
     * corner.
     * 
     * @param rectangle the target {@code Rectangle} that will be modified by
     *        this operation
     * @param scale the amount that the {@code Rectangle} will be scaled by
     * @see Geometry#scale(Dimension, double)
     * @see Geometry#alignCenter(Rectangle, Point)
     */
    public static void scale(Rectangle rectangle, double scale) {
        Dimension dimension = rectangle.getSize();
     ...
     * given point.
     * 
     * @param rectangle the {@code Rectangle} that will be aligned
     * @param center the {@code Point} that the {@code Rectangle} will be
     *        centered on
     */
    public static void alignCenter(Rectangle rectangle, Point center) {
        Dimension halfDim = rectangle.getSize();
     ...
     * 
     * @param dimension the {@code Dimension} that will be halved as a result of
     *        this operation
     * @see Geometry#scale(Dimension, double)
     */
    public static void half(Dimension dimension) {
     ...
     * 
     * @param dimension the {@code Dimension} that will be flipped as a result
     *        of this operation
     * @see Geometry#scale(Dimension, double)
     */
    public static void flip(Dimension dimension) {
     ...
     *        of this operation
     */
    public static void flipOverY(Dimension dimension) {
     ...
     *        of this operation
     */
    public static void flipOverX(Dimension dimension) {
        Geometry.scale(dimension, 1.0d, -1.0d);
    }
     ...
     * 
     * @param source the source, or starting, {@code Point}
     * @param destination the final, or end, {@code Point}
     * @param interpolation the percent that the created point should use to
     ...
     *        {@code destination} <li>{@code 2.0} will make the created point be
     *        on the line made by the two specified points, but twice as far
     *        away from {@code source} as {@code destination} currently is.
     * @return the interpolated {@code Point}
     */
    public static Point interpolate(Point source, Point destination, double interpolation) {
        Dimension bounds = Geometry.bounds(source, destination);
        int interpolatedX = (int)(source.x + (bounds.width * interpolation));
        int interpolatedY = (int)(source.y + (bounds.height * interpolation));
     ...
     * zero.
     * 
     * @param rectangle the {@code Rectangle } that will be trimmed during this
     *        operation. Negative values for width and height are supported.
     * @param insets the {@code Insets} to trim with. Negative values for the
     *        {@code Insets} are allowed, but will grow the {@code Dimension}
     *        instead of trimming it.
     */
    public static void subtractInsets(Rectangle rectangle, Insets insets) {
        Dimension insetDim = Geometry.bounds(insets);
     ...
     * zero.
     * 
     * @param dimension the {@code Dimension } that will be trimmed during this
     *        operation. Negative values for width and height are supported.
     * @param insets the {@code Insets} to trim with. Negative values for the
     *        {@code Insets} are allowed, but will grow the {@code Dimension}
     *        instead of trimming it.
     */
    public static void subtractInsets(Dimension dimension, Insets insets) {
        Dimension insetDim = Geometry.bounds(insets);
     ...
     * 
     * @param a the first numeric bound
     * @param b the second numeric bound
     ...
     */
    public static boolean containsInclusive(double a, double b, double candidate) {
        if(a > b) {
     ...
     */
    public static boolean containsExclusive(double a, double b, double candidate) {
        if(a > b) {
            double temp = a;
            a = b;
            b = temp;
     ...
     * will be negative.
     * 
     * @param source the starting {@code Point}
     * @param destination the end {@code Point}
     * @return The {@code Dimension} that is equivalent to the distance between
     *         the specified {@code Point} objects.
     */
    public static Dimension bounds(Point source, Point destination) {
     ...
     * undefined.
     * 
     * @param insets the {@code Insets} that are used in this operation
     * @return The bounds, equal to {@code (left + right, top + bottom)}
     */
    public static Dimension bounds(Insets insets) {
     ...
     * all sides.
     * 
     * @param magnitude the value that the created {@code Insets} object will
     *        use for every side
     * @return the created {@code Insets} object
     */
    public static Insets insets(int magnitude) {
     ...
     * 
     * @param widthMagnitude the value of the left and right {@code Insets}
     *        values
     * @param heightMagnitude the value of the top and bottom {@code Insets}
     *        values
     * @return the created {@code Insets} object
     */
    public static Insets insets(int widthMagnitude, int heightMagnitude) {
     ...
     * 
     * @param rectangle the {@code Rectangle} used in this operation
    ...
    /**
     * Returns a {@link Dimension} which has an hypotenuse equal to the
     * specified one.
     * 
     * @param hypotenuse the hypotenuse of the created {@code Dimension}
     * @return a {@Dimension} as described above
     */
    public static Dimension getDimensionFromHypotenuse(double hypotenuse) {
        int side = (int)Math.sqrt(Math.pow(hypotenuse, 2) / 2);
